...
void vPort_Init(void){ 
    ...
    P1&=!0x0F;                          // turn P1.0-3 outputs low
...
int iScale(int measurement){
     ...
     * where b=9, a=0, x=measurement, min and max are known hence:           *
     * f(x)=(9*(measurement-min))/(max-min)                                  *
     *                                                                       *
    ...
    scaled=(int)(9.0*(measurement-min))/(max-min);
